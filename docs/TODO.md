...
- **Counter integration** (v1.1.0)
  ...
  - Bridge architecture: page-context script intercepts `fetch()` and SSE streams, relays data via `window.postMessage`
  - Uses vendored `o200k_base` tokenizer (~2MB) for approximate token counting
  - Isolated namespace (`globalThis.ClaudeCounter` / `.cc-*` CSS) — no conflicts with exporter code
  - Added `counter/` (7 files) and `vendor/o200k_base.js` to both Chrome and Firefox builds
  - Updated manifests, background scripts, and web_accessible_resources

- **Per-chat subfolders in batch ZIPs** (v1.10.18)
  - Export All, Browse export, Project export: each chat now gets its own subfolder (`ChatName/ChatName.md`)
  - Consistent structure across all three export modes and all artifact settings

- **Uploaded file export (wiggle files)** (v1.10.18)
  - Files uploaded to Claude.ai (images, PDFs, Excel, etc.) are fetched from the wiggle API and included in `attachments/` subfolder
  - Covers all export modes: single-chat Export Current, Export All, Browse export, Project export
  - Flat mode prefixes uploaded files with conversation name to avoid collisions
  - Pasted text snippets (from message.attachments) also saved as individual files in `attachments/`

- **Export Project** (v1.10.18)
  - Export a full project ZIP from the browse page including chats, artifacts, and knowledge files
  - Per-chat subfolders in `context/chats/` with artifacts in `context/artifacts/`

- **Import Backup: ask mode before opening file picker** (v1.10.17)
