...
Complete installation instructions for ClawdKit on Chrome and Firefox.
...
1. Download the `claude-exporter-chrome-vX.X.X.zip` from the [Releases page](https://github.com/pavan-kalyan-ai/claude-ai-chat-exporter/releases)
...
6. Done! See [Configuration](#configuration) only if auto-detection fails

### Firefox

1. Download the latest `.xpi` file from the [Releases page](https://github.com/pavan-kalyan-ai/claude-ai-chat-exporter/releases)
...
1. **Clone or Download the Repository**
   ```bash
   git clone https://github.com/pavan-kalyan-ai/claude-ai-chat-exporter.git
   cd claude-exporter
   ```
...
5. **See [Configuration](#configuration) only if auto-detection fails**
...
3. **See [Configuration](#configuration) only if auto-detection fails**
...
## Token Counter & Usage Bars

Once installed, the extension automatically shows the following on claude.ai — no configuration needed:

- **Token count** — Approximate token count and mini progress bar next to the conversation header (200k context limit)
- **Cache timer** — Countdown showing how long the conversation remains cached (cheaper to continue)
- **Usage bars** — Session (5h) and weekly (7d) usage bars near the model selector, with reset countdowns

These features use Claude's native API and live SSE data. Click the usage bars to manually refresh.

---

## Configuration

The extension automatically detects your Organization ID from your active Claude.ai session — no manual setup required for most users.

**If auto-detection fails** (e.g. you see "Could not detect Organization ID"):

1. Right-click the extension icon → **Options** (or click the gear icon in the popup)
2. In a new tab, go to `https://claude.ai/settings/account`
...
4. Return to the extension options and paste the Organization ID
5. Click **Save**
...
- Make sure you're logged into Claude.ai and have a claude.ai tab open
- If the problem persists, set it manually via the Options page — see [Configuration](#configuration) above
...
2. Find "ClawdKit" and click **Remove**
