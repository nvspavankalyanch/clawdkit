...
3. **ZIP Chrome extension** — `cd chrome && zip -r ../releases/vX.Y.Z/claude-exporter-chrome.zip ./* && cd ..`
4. **ZIP Firefox extension** — `cd firefox && zip -r ../releases/vX.Y.Z/claude-exporter-firefox.zip ./* && cd ..` (unsigned; user handles .xpi signing via AMO)
5. **Git tag** — `git tag vX.Y.Z -m "Release vX.Y.Z"`
